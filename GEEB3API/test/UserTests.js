...
describe('User testing', () => {
  ...
  describe('/PUT user /users/update/:id', function () {
    it('should update the new user created and return a status 200', function () {
      ...
      const updatedUser =
        '{"email": "test@Arquitectura.com", "password": "testPassword1"}';
      axios.put(
        {
  ...
  describe('/GET user /users/mail-query/:username', function () {
    it('should return the email of the user and return a status 200', function (done) {
      ...
      axios.get(
        baseURL + '/users/mail-query/' + username,
        (error, response, body) => {
          if (error) done(error);
          assert.equal(response.statusCode, 200);
          done();
  ...
  describe('POST users /by-username', function () {
    ...
    it('Status 400', (done) => {
      ...
      axios.post(baseURL + '/by-username').then((res) => {
  ...
  describe('POST users /by-email', function () {
    ...
    it('Status 200', (done) => {
      ...
      axios.post(baseURL + '/by-email', { email: 'arqui@test' }).then((res) => {
    ...
    it('Status 400', (done) => {
      axios.post(baseURL + '/by-email').then((res) => {
        assert.equal(res.status, 400);
      });
      done();
    });
    it('Status 404', (done) => {
      axios.post(baseURL + '/by-email', { email: 'arki@test' }).then((res) => {
  ...
  describe('POST users /register', function () {
    it('should create a new user and return a status different from 500 (error)', async function () {
      ...
      const userRegisered = await axios.post(baseURL + '/register', newUser);
